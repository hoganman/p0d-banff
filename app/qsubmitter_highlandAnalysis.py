...
inOptions = {
        ...
        'a:':['app=','the highland2 application name like RunP0DNumuCCQEAnalysis.exe'],
...
def CreateHL2SubmissionScript(jobNum,priority,walltimeHours,walltimeMinutes,maxMemory,softRequirements,hardRequirements,useQueueName,useHostName,otherRequirementsList,emailAddress):
...
def CreateHL2JobScript(application,jobNum,subFileList,outputPath,outputName):
    #create filelist
    CWD = os.getcwd()
    ...
    job.write('FILEGOOD=$?\n')
    job.write('if [ $FILEGOOD -eq 0 ]; then\n')
    job.write('  echo "The HL2 failed! There was some kind of crash!"\n')
    job.write('  exit 1\n')
    job.write('fi\n')
    job.write('\n')
    job.close()
...
def MakeJobs(outputPath,outputName,application,numJobs,numFilesPerJob,priority,walltimeHours,walltimeMinutes,maxMemory,softRequirements,hardRequirements,useOpportunisticNodes,useQueueName,useHostName,otherRequirementsList,emailAddress,ExportedPathsName):
    ...
    for jobNum in range(0,numJobs):
        ...
        CreateHL2SubmissionScript(jobNum+1,priority,walltimeHours,walltimeMinutes,maxMemory,softRequirements,hardRequirements,useQueueName,useHostName,otherRequirementsList,emailAddress)

        #create job script
        HL2s.append(outputFile)
        CreateHL2JobScript(application,jobNum+1,subFileList,outputPath,outputName)
...
def main(argv):
    ...
    if clusterName not in ('csuhpc','csuhep'):
        ...
        print helpstatement
        sys.exit()
    possibleApplications = subprocess.Popen(['which',applicationName],stdout=subprocess.PIPE).communicate()[0].strip()
    if 'which: no' in possibleApplications:
        print 'ERROR: Unable to find application ' + applicationName
        print helpstatement
        sys.exit()
    application = possibleApplications
    print 'Using application:', application
    ...
    submissionDirName = 'qsubmitter_%s_%s_%d%s%s_%s%s'%(outputName,applicationName, currentTime.year,str(currentTime.month).zfill(2),str(currentTime.day).zfill(2),str(currentTime.hour).zfill(2),str(currentTime.minute).zfill(2))
    ...
    MakeJobs(outputPath,outputName,application,numJobs,numFilesPerJob,priority,walltimeHours,walltimeMinutes,maxMemory,softRequirements,hardRequirements,useOpportunisticNodes,useQueueName,useHostName,otherRequirementsList,emailAddress,ExportedPathsName)
