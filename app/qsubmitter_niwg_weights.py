...
inOptions = {
        ...
        'r:':['run_type=','is MC or Sand (assumes MC)'],
...
APPLYNIWGWEIGHTS = '%s/app/genWeightFromPsycheRunSyst_new.exe' % (os.getenv('T2KREWEIGHT'))
...
GenWeightss = []
fileList = {} # a dictionary with a binnCorr file as the key and 0(unused) or 1(used) as the value
totalTimeInHours = 0
...
def MakeJobs(outputPath,outputName,numJobs,numFilesPerJob,priority,walltimeHours,walltimeMinutes,maxMemory,softRequirements,hardRequirements,useOpportunisticNodes,useQueueName,useHostName,otherRequirementsList,emailAddress,ExportedPathsName):
    ...
    global fileList
    binnCorrFiles = fileList.keys()
    NaturalSorting.naturalsort(binnCorrFiles)
    for jobNum in range(0,numJobs):
        ...
        while fileCounter < numFilesPerJob:
            if globalFileListIndex == len(fileList):
                break
            abinnCorrFile = binnCorrFiles[globalFileListIndex]
            if fileList[abinnCorrFile] == 1:
                print "You should not be calling an already used binnCorr file"
                print "   going to the next one "
            else:
                subFileList.append(abinnCorrFile)
                fileList[abinnCorrFile] = 1
...
def main(argv):
    ...
    if numFilesPerJob > 0 and numJobs == -1:
        numJobs = int(math.ceil(float(nFiles) / numFilesPerJob))
    global isSand
    if run_type not in ('SAND','sand','Sand') and run_type not in ('MC', 'mc', 'Mc'):
