...
public:
  ...
  UInt_t GetNParameters(UInt_t index) const {
    if (!GetToyVariations(index)){
      std::cout << " ToyExperiment::GetNParameters(): variation with index " << index << " does not exist " << std::endl;   
    }
    return GetToyVariations(index)->GetNParameters();
  } 
  ...
  void SetToyVariation(UInt_t index, UInt_t ipar, Float_t var, Float_t weight=1.) {
    ...
    _toyVariations[index]->Variations[ipar] = var;_toyVariations[index]->Weights[ipar] = weight; 
  } 
