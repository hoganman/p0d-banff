...
//***********************************************************
    ...
    // True by default --> to review
    _applyWeightsWhenAllCutsPassed = true;
}
  ...
  for (std::vector<SelectionBase*>::iterator sit= sel.GetSelections().begin();sit!=sel.GetSelections().end();sit++){
    ...
    for (UInt_t j = 0; j < _eventWeights.size(); ++j){
      if (_eventWeights[j]->IsEnabled())
        _eventWeights[j]->InitializeSystBoxes(sel.GetNEnabledSelections(),selec->GetEnabledIndex(), selec->GetNBranches(), nevents);
    }
  }
    //TODO
}
...
//***********************************************************
Weight_h EventWeightManager::ComputeEventWeightsWithSystId(const SelectionBase& sel, const ToyExperiment& toy, const AnaEventC& event, const ToyBoxB& ToyBox, Weight_h* weights) {
//***********************************************************
  ...
  for(UInt_t i = 0; i<NMAXSYSTEMATICS; ++i) weights[i] = -999;
  ...
  for (UInt_t j = 0; j < _eventWeights.size(); j++) {
    ...
    // At least on of the branches should be succesful. Otherwise ibranch is
    // ToyBoxB::kNoSuccessfulBranch
    // There is a parameter to control whether to allow applying systematics
    // even if no branch has all the cuts passed (will review whether it is
    // really needed)
    if ((ibranch != ToyBoxB::kNoSuccessfulBranch || !_applyWeightsWhenAllCutsPassed) && sel.IsRelevantSystematic(event, ToyBox, _eventWeights[j]->GetIndex(), ibranch)){          
#ifndef MULTITHREAD
...
#endif
      UInt_t index = _eventWeights[j]->GetIndex();
      weights[index] = _eventWeights[j]->ComputeWeight(toy, event, ToyBox, sel);
