...
const UInt_t NMAXSELECTIONS=30;
...
public:
  ...
  //---- Keep functions mandatory, an anlyser will implement them for possible  ----
  //---- usage by fitters  ----
  /// Fill the Event Summary, needed by fitters
  virtual bool FillEventSummary(AnaEventC&, Int_t*){
    std::cout << "ERROR: void FillEventSummary() must be implemented in your Selection!!!" << std::endl;
    exit(1);
  }// = 0;

  /// Return the sample type, needed by fitters
  virtual SampleId_h GetSampleId(){
    std::cout << "ERROR: SampleId_h GetSampleId() must be implemented in your Selection!!!" << std::endl;
    exit(1);
    return UNASSIGNEDID;
  }// = 0;
  
  /// Get the IDs of the RecObject groups that are relevant for a given systematic and branch in a given selection
  virtual Int_t GetRelevantRecObjectGroupsForSystematic(SystId_h, Int_t*, Int_t) const{
    std::cout << " ERROR:Int_t GetRelevantRecObjectGroupsForSystematic() must be implemented in your Selection "<< typeid(*this).name() << " !!! " << std::endl;
    exit(1);
  };
  
  /// Get the IDs of the TrueObject groups that are relevant for a given systematic and branch in a given selection
  virtual Int_t GetRelevantTrueObjectGroupsForSystematic(SystId_h, Int_t*, Int_t) const{
    std::cout << " ERROR:Int_t GetRelevantTrueObjectGroupsForSystematic() must be implemented in your Selection "<< typeid(*this).name() << " !!! " << std::endl;
    exit(1);
  };
  //==========================================================
  ...
  void InitializeRootStepFromFirstSteps();
  ...
  // Initialize toy,  not obligatory but important in case a selection needs
  // to do something specific for the toy experiment: e.g. re-order tracks based on the parameter that
  // is subject of a change by toy 
  //-----------------------------------------------------------------
  virtual void InitializeToy()const{}
    
  // Finalize toy,  not obligatory but important in case a selection needs
  // to do something specific for the toy experiment. The function is called to be able to set back any
  // changes done by IntializeToy
  //-----------------------------------------------------------------
  virtual void FinalizeToy()const{}
...
//  void DisableStep(const std::vector<UInt_t>& branch_seq, UInt_t step_nbr);
  ...
  /// Get all steps in a given branch provided the branch Unique ID.
  /// This method uses a different algorithm and serves to validate the algorith used by all others methods above
  ...
  virtual void SetDetectorFV(SubDetId_h det, Int_t ibranch=-1);
  ...
  /// Return an step provided the title. returns as argument the branch ID
  StepBase* GetStepAndBranchID(const std::string& title, Int_t& ID) const;

  /// Return the previous step provided the title and the Branch ID
...
//  UInt_t GetNSteps(Int_t branch);
  ...
  bool IsValidRun(int runPeriod) const {
    ...
    try {  
      return _validRunPeriods.at(runPeriod);
    }
    catch (const std::out_of_range& oor) {
      // This run period is not know hence will be ignored
      return false;
    } 
    return false;
  }
...
protected:
  ...
  /// Boolean vector to store valid run periods for this selection
  std::vector<bool> _validRunPeriods; //!
