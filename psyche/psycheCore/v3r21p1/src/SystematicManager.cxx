...
//***********************************************************
  ...
  for (int j = 0; j < _nVariationSystematics; ++j){
    if (_variationSystematics[j]->IsEnabled())
      _variationSystematics[j]->InitializeSystBoxes(1,0,1, nevents);
  }
  ...
  for (std::vector<SelectionBase*>::iterator sit= sel.GetSelections().begin();sit!=sel.GetSelections().end();sit++){
    ...
    for (int j = 0; j < _nWeightSystematics; ++j){
      if (_weightSystematics[j]->IsEnabled())
        _weightSystematics[j]->InitializeSystBoxes(sel.GetNEnabledSelections(),selec->GetEnabledIndex(), selec->GetNBranches(), nevents);
    }
  }
    //TODO
}
...
//***********************************************************
    ...
    // Additional initializtion that may be required for a systematic after it
    // gets an index
    sys->Initialize();
