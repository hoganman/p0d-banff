...
Bool_t PionSIManager::InVOI(SubDetId_h det, const Float_t* pos) const{  
...
Bool_t PionSIManager::InVOI1(const Float_t* pos) const{
...
Bool_t PionSIManager::InVOI2(const Float_t* pos) const{
...
Bool_t PionSIManager::InVOIext(const Float_t* pos) const{
...
//********************************************************************
Bool_t PionSIManager::CheckLookForInteraction(SubDetId_h det, const AnaTrueParticleB& particle) const{  
  //********************************************************************
  ...
  return (InVOI(det, particle.PositionEnd));
...
#elif JMDEBUGVERSION == 1
  (void)det;
  return ((InVOI1(particle.Position ) && InVOI1(particle.PositionEnd )) ||
      (InVOI2( particle.Position ) && InVOI2(particle.PositionEnd)));
#endif
...
PionInteractionSystematic* PionSIManager::ComputePionWeightInfo(const AnaEventB& event, Int_t det) const{
  ...
  for (UInt_t p = 0; p < pionTrajs.size(); p++){
    ...
    if (CheckLookForInteraction(det, *pionTraj)){
