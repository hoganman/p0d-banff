...
//
// 2013-06-27: !! THIS APP IS DEPRECATED, USE genWeights_2013a.cxx FROM NOW !!
//
///////////////////////////////////////////////////////////////////////////////////
...
{
#if defined (__T2KRW_JNUBEAM_ENABLED__) && defined (__T2KRW_NIWG_ENABLED__)
  ParseArgs(argc, argv);
  ...
  jnubeam::rew::JReWeightEnu2012a* jrw_ptr = (jnubeam::rew::JReWeightEnu2012a*)((t2krew::T2KJNuBeamReWeight*)rw.WghtEngine("jnubeam_rw"))->GetWghtCalc("enu2012a");
  if (!jrw_ptr) {
    cout << endl << "Error: \"enu2012a\" WghtCalc not enabled in \"jnubeam_rw\" WghtEngine" << endl;
    exit (-1);
  }
  ...
  // Check axis binning against the hardcoded ones in JReWeightEnu2012a.cc
  ...
  for (int ipar=0; ipar<nPars; ipar++) {
    ...
    TAxis *axis;
    if (ipar<=lastFluxPar) axis = jrw_ptr->axis[ipar];
	...
	// Keep count of flux bins for the other detector, if present in parameter file
	  ...
	  continue;
        ...
        for (int ibin=0; ibin<parBins[ipar]->GetNbins(); ibin++) {
          ...
          string systString = Form("JEnu2012a_%s%s%d",jnuDet[jnuDetType].c_str(),jnuFlux[jnuFluxType].c_str(),ibin);
...
void Usage(){
  ...
  cout << "genWeights_2012a.exe -i <Input Filename> -p <Input BANFF Parameter Filename> -o <Output Weight Tree Filename>" << endl;
  exit(1);
}
