...
int main(int argc, char *argv[])
{
    ...
    for(int i=0; i<5; i++) maqe_vec.push_back(ma_spacing[i]*(0.3/1.2));
    dial_values.push_back(maqe_vec);
    dials.push_back(t2krew::kNXSec_MaCCQE);
    dial_names.push_back("MAQE");
    TClonesArray *maqe_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    ...
    TClonesArray *q3cut_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    ...
    TClonesArray *mecc_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    dial_graphs.push_back(mecc_graph);

    //PF_C dial values;
    std::vector<float> pf_c;
    for(int i=0; i<5; i++) pf_c.push_back((i - 2)*(100./(6.*217.)) + (475./(2.*217.) - 1.));
    dial_values.push_back(pf_c);
    dials.push_back(t2krew::kNIWG2014a_pF_C12);
    dial_names.push_back("PF_C");
    TClonesArray *pfc_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    ...
    TClonesArray *ebc_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    ...
    TClonesArray *meco_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    dial_graphs.push_back(meco_graph);

    //PF_O dial values;
    std::vector<float> pf_o;
    for(int i=0; i<5; i++) pf_o.push_back((i - 2)*(100./(6.*225.)) + (475./(2.*225.) - 1.));
    dial_values.push_back(pf_o);
    dials.push_back(t2krew::kNIWG2014a_pF_O16);
    dial_names.push_back("PF_O");
    TClonesArray *pfo_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
...
//    TClonesArray *coh_bs_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
//    dial_graphs.push_back(coh_bs_graph);
    ...
    TClonesArray *sfrfg_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    ...
    TClonesArray *rpa_graph = new TClonesArray("TGraph",nd280Samples::kNSamples);
    dial_graphs.push_back(rpa_graph);

    //Set up custom summary trees
    Double_t Enu[nd280Samples::kNSamples],Pmu[nd280Samples::kNSamples],CosThetamu[nd280Samples::kNSamples],FluxWeight[nd280Samples::kNSamples],DetWeight[nd280Samples::kNSamples],POTWeight[nd280Samples::kNSamples],TruePmu[nd280Samples::kNSamples],TrueCosThetamu[nd280Samples::kNSamples],Q2[nd280Samples::kNSamples],Q2QE[nd280Samples::kNSamples],PileUpWeight[nd280Samples::kNSamples],CCQETuningWeight[nd280Samples::kNSamples], MaQETuningWeight[nd280Samples::kNSamples], MECOTuningWeight[nd280Samples::kNSamples], MECCTuningWeight[nd280Samples::kNSamples], pFTuningWeight[nd280Samples::kNSamples], RFGTuningWeight[nd280Samples::kNSamples], RelRPATuningWeight[nd280Samples::kNSamples], NonRelRPATuningWeight[nd280Samples::kNSamples], CohTuningWeight[nd280Samples::kNSamples];
    Int_t NeutrinoCode[nd280Samples::kNSamples],ReactionCode[nd280Samples::kNSamples],TgtMat[nd280Samples::kNSamples],NuParent[nd280Samples::kNSamples],Run,EventType[nd280Samples::kNSamples],TruthVtx[nd280Samples::kNSamples];
    Bool_t HaveTruth[nd280Samples::kNSamples];
    ...
    _man.input().Initialize(fInputFile,"");

    //Get the number of entries in the input tree.
    Int_t nentries = (Int_t)_man.input().GetEntries();

    std::cout << nentries << " entries in input tree." << std::endl;

    std::cout << _man.GetEntries() << " loaded by AnalysisManager" << std::endl;
    ...
    std::cout << "For Run Period " << fRunPeriod << " Run = " << Run << std::endl;
    ...
    //Need to set up systematics to process event.  Just do nominal.
    float fluxweights;
    float detweights;
    ...
    Int_t NSamples = nd280Samples::kNSamples;
    ...
    ssum_tree.Branch("DetWeight", DetWeight, "DetWeight[NSamples]/D");
    ssum_tree.Branch("POTWeight", POTWeight, "POTWeight[NSamples]/D");
    ssum_tree.Branch("PileUpWeight", PileUpWeight, "PileUpWeight[NSamples]/D");
    ...
    ssum_tree.Branch("EventType", EventType, "EventType[NSamples]/I");
    ...
    for(int q = 0; q < nentries; ++q){
        ...
        hdr->GetEntry(q);
        if(q == 0) hdr1->Fill();
        AnaEventB* event = _man.LoadSuperEvent(q)->Event;
        ...
        passed = _man.ProcessEvent((*toy),*event,detweights,fluxweights);
        eventsProcessed++;
        ...
        if(!data){
            ...
            for(int sample = 0; sample < nd280Samples::kNSamples; ++sample){

                if(event->Summary->TrueVertex[sample] != NULL && event->Summary->EventSample == sample){
                    //Save stuff  
                    Pmu[sample] = event->Summary->LeptonCandidate[sample]->Momentum;
                    CosThetamu[sample] = event->Summary->LeptonCandidate[sample]->DirectionStart[2];
                    TruthVtx[sample] = event->Summary->TrueVertex[sample]->TruthVertexID; 
                }
                else{
                ...
                DetWeight[sample] = 1.0;
                Enu[sample] = -999.;
                ReactionCode[sample] = -999.;
                Double_t weight = 1.0;
                EventType[sample] = 0; //EventType zero corresponds to undefined (because selection did not make it to CCInc cut)
                // We are not using the EventType variable anymore, but I'll leave this here just in case we use it in the future
                if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCC0Pi) EventType[sample] = 1;
                else if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCC1Pi) EventType[sample] = 2;
                else if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCCOther) EventType[sample] = 3;
                else if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCC) EventType[sample] = 4;
                else if(event->Summary->EventSample == nd280Samples::kFGD2NuMuCC) EventType[sample] = 5;
                ...
                if(TruthVtx[sample] > -1){
                    ...
                    while(vtx == NULL){
                        ...
                        for(int i = 0; i < NRooVtx; ++i){
                            ...
                            if(vtx->TruthVertexID == TruthVtx[sample] && fabs(vtx->StdHepP4[0][3]*1000 - event->Summary->TrueVertex[sample]->NuEnergy) < 0.1) break;
                            vtx = NULL;
                        }
                        if(vtx == NULL) rtvi++;
                        if(rtvi==rtv_entries){
                ...
                if(!vtx || Run==-1){
                    ...
                    if(fabs(ReactionCode[sample]) == 16){
                        for(int i = 0; i < event->nTrueTracks; ++i){
                            if(fabs(event->TrueTracks[i]->PDG) != 211) continue;
                            if(event->TrueTracks[i]->TrueVertex == NULL) continue;
                            if(event->TrueTracks[i]->TrueVertex->TruthVertexID == event->Summary->TrueVertex[event->Summary->EventSample]->TruthVertexID){
                                piEnergy = sqrt(event->TrueTracks[i]->Momentum*event->TrueTracks[i]->Momentum + 139.57*139.57)/1000.0;
                            }
                        }
                        if(piEnergy < 0){
                            ...
                            CohTuningWeight[sample] = 0.135;
                        }
                        else if(piEnergy < 0.5){
                            CohTuningWeight[sample] = 0.4;
                        }
                        else if(piEnergy < 0.75){
                            CohTuningWeight[sample] = 0.294;
                        }
                        else if(piEnergy < 1.0){
                            CohTuningWeight[sample] = 1.206;
                        }
                    }
                ...
                if(Run == -1){
                    ...
                    rw.Systematics().SetTwkDial(t2krew::kNIWG2014a_pF_C12, (223.-217.)/217.);
                    rw.Systematics().SetTwkDial(t2krew::kNIWGMEC_Norm_C12, 0);
                    rw.Systematics().SetTwkDial(t2krew::kNIWGMEC_Norm_O16, 0);  
                    rw.Systematics().SetTwkDial(t2krew::kNXSec_MaCCQE, 0);

                    rw.Reconfigure();
                    pFTuningWeight[sample] = rw.CalcWeight(vtx);

                    rw.Systematics().SetTwkDial(t2krew::kNIWG2014a_pF_C12, 0);
                    rw.Systematics().SetTwkDial(t2krew::kNIWGMEC_Norm_C12, (0.27-1.0)/1.0);

                    rw.Reconfigure();
                    MECCTuningWeight[sample] = rw.CalcWeight(vtx);

                    rw.Systematics().SetTwkDial(t2krew::kNIWGMEC_Norm_C12, 0);
                    rw.Systematics().SetTwkDial(t2krew::kNIWGMEC_Norm_O16, (0.27-1.0)/1.0);  

                    rw.Reconfigure();
                    MECOTuningWeight[sample] = rw.CalcWeight(vtx);

                    rw.Systematics().SetTwkDial(t2krew::kNIWGMEC_Norm_O16, 0);  
                    rw.Systematics().SetTwkDial(t2krew::kNXSec_MaCCQE, (1.15-1.2)/1.2);

                    rw.Reconfigure();
                    MaQETuningWeight[sample] = rw.CalcWeight(vtx);
                }
        ...
        //Fill summary tree
        std::cout << "Time to fill!!" << std::endl;
...
void Usage(){
    ...
    std::cout << "genWeightsFromNRooTracker_BANFFv2_2014.exe -p psyche_inputfile -r run_period -o outputfile -d data_bool" << std::endl;
    std::cout << "psyche_inputfile is a flat tree suitable for input into psyche." << std::endl;
    std::cout << "run_period is one of: 2a 2w 3b 3c 4a 4w sand mec" << std::endl;
    std::cout << "outputfile is the desired name of the output file." << std::endl;
    std::cout << "data_bool is a boolean to denote whether we are running on data or not." << std::endl;
...
void ParseArgs(int argc, char **argv){
    ...
    for(int i = 1; i < argc; i+=2){
        ...
        else if(string(argv[i]) == "-d") data = atoi(argv[i+1]);
