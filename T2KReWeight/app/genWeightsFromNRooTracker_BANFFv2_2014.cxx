...
int main(int argc, char *argv[])
{
 ...
 flux_file[4] = new TFile("run4_full/nd5_tuned11bv3.2_11anom_run4_fine.root");
 TH1D *flux_hist[5][4];
 for(int i=0; i<5; i++)
   for(int j=0; j<4; j++)
...
//****    }  
   ...
   //Forget the file_list.  Only dealing with one file for now.
   ...
   _man.ReadEvents(fInputFile,nentries);

    std::cout << _man.GetEntries() << " loaded by AnalysisManager" << std::endl;

  //While we're dealing with the file here, load in the NRooTrackerVtx tree
  //here too.
  ...
  //For getting the correct RooTrackerVtx entry.
  //Set the previous event number to the first event in the event list.
  Int_t prevEventNum = _man.GetEvent(0)->RawEvent->EventInfo.Event;
  ...
  // Fixed 0 sigma for all weight systematics
  for (UInt_t isyst = toy.GetNVariationSystematics(); isyst<toy.GetNVariationSystematics()+toy.GetNWeightSystematics();isyst++)
...
//
//        bool passed = _man.ProcessEvent(toy,systbox,*(_man.GetEvent(i)->RawEvent),weights);
//
//        if(!passed){
//            continue;
//        }
//
//        //std::cout <<  "RooVtxIndex value from source: " << _man.GetEvent(i)->RawEvent->Summary->Vertex->TrueVertex->RooVtxIndex << std::endl;
//
//    }
   ...
   //for( unsigned int i = 0; i < 100; i++ ) {
    ...
    //Process the event.
    //passed indicates whether a selection was passed or not
       //std::cout << eventsProcessed << std::endl;
    bool passed = _man.ProcessEvent(toy,systbox,*(_man.GetEvent(i)->RawEvent),weights);
    eventsProcessed++;
    
    //If ProcessEvent has no event summary, (does not have a found vertex and
    //HMN track) skip it.
    if(!(_man.GetEvent(i)->RawEvent->Summary)){
        continue;
    }
    //std::cout << "Selection passed!" << std::endl;
     ...
     //Save stuff   
     Double_t weight = 1.0;
     Pmu = _man.GetEvent(i)->RawEvent->Summary->LeptonCandidate->Momentum;
     CosThetamu = _man.GetEvent(i)->RawEvent->Summary->LeptonCandidate->DirectionStart.CosTheta();
     EventType = 0; //EventType zero corresponds to undefined (because selection did not make it to CCInc cut)
     if(_man.GetEvent(i)->RawEvent->Summary->EventSample == nd280Samples::kFGD1NuMuCC0Pi) EventType = 1;
     else if(_man.GetEvent(i)->RawEvent->Summary->EventSample == nd280Samples::kFGD1NuMuCC1Pi) EventType = 2;
     else if(_man.GetEvent(i)->RawEvent->Summary->EventSample == nd280Samples::kFGD1NuMuCCOther) EventType = 3;
     int flavor = 0;
     FluxWeight = 1.0;
     Enu = 0.;
     ReactionCode = -999.;
     //****TruthVtx = events[i].Stat.TruthVertexID;    
     TruthVtx = _man.GetEvent(i)->RawEvent->Summary->Vertex->TrueVertex->TruthVertexID; 


    //What we need to get the RooTrackerVtx is its array index in the
    //TClonesArray, which is given by AnaTrueVertexB::RooVtxIndex
    Int_t RooVtxIndex = _man.GetEvent(i)->RawEvent->Summary->Vertex->TrueVertex->RooVtxIndex;
    //std::cout << "RooVtxIndex value: " << RooVtxIndex << std::endl;
     //Get the nRooTrackerVtx
     //****ND::NRooTrackerVtx * vtx = (ND::NRooTrackerVtx*)(events[i].NRooTrackerVtx);
     //
     //Must be carefuly getting the RooTracker vertex, as the flattree entries
     //are bunched, while the RooTracker vertex is not.
     
     //If the event number has changed between the previous event and this one,
     //increment the index used to get the RooTrackerVtx tree entry.
      if(prevEventNum != _man.GetEvent(i)->RawEvent->EventInfo.Event){

        prevEventNum = _man.GetEvent(i)->RawEvent->EventInfo.Event;
        rtvi++;
      }
      ...
      //event.
      //std::cout << "The problem should be below" << std::endl;
      ND::NRooTrackerVtx * vtx = (ND::NRooTrackerVtx*)(*nRooVtxs)[RooVtxIndex];
      //  std::cout << "The problem should be above" << std::endl;

        //std::cout << "Neutrino interaction vertex PDG: " << vtx->StdHepPdg[1] << std::endl;
...
void Usage(){
  ...
  cout << "genWeightsFromNRooTracker_BANFFv2_2014.exe -p psyche_inputfile -r run_period -o outputfile" << endl;
  cout << "psyche_inputfile is a flat tree suitable for input into psyche." << endl;
  cout << "run_period is one of: 2a 2w 3b 3c 4a 4w sand mec" << endl;
  cout << "outputfile is the desired name of the output file." << endl;
}
