...
int main(int argc, char *argv[])
{
    ...
    _man.SetNEventsToProcess(nentries);
    ...
    Int_t prevEventNum = -1; //_man.GetSuperEvent(0)->RawEvent->EventInfo.Event;
    ...
    while(true) {
        AnaEventB* event = _man.GetNextEvent();
        if (!event) break; // exit the loop when there are no more events

        //Process the event.
        //passed indicates whether a selection was passed or not
        //std::cout << eventsProcessed << std::endl;
        float fluxWeight;
        bool passed = _man.ProcessEvent(toy,*event,weights, fluxWeight);
        eventsProcessed++;
        ...
        if(prevEventNum != event->EventInfo.Event){
            rtvi += event->EventInfo.Event - prevEventNum;
            prevEventNum = event->EventInfo.Event;
        }
        ...
        //Save stuff   
        Double_t weight = 1.0;
        Pmu = event->Summary->LeptonCandidate->Momentum;
        CosThetamu = event->Summary->LeptonCandidate->DirectionStart[2];
        EventType = 0; //EventType zero corresponds to undefined (because selection did not make it to CCInc cut)
        if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCC0Pi) EventType = 1;
        else if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCC1Pi) EventType = 2;
        else if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCCOther) EventType = 3;
        else if(event->Summary->EventSample == nd280Samples::kFGD1NuMuCC) EventType = 4;
        else if(event->Summary->EventSample == nd280Samples::kFGD2NuMuCC) EventType = 5;
        int flavor = 0;
        FluxWeight = 1.0;
        Enu = 0.;
        ReactionCode = -999.;
        //****TruthVtx = events[i].Stat.TruthVertexID;   
        std::cout << event->Summary->Vertex->PrimaryIndex << std::endl;

        TruthVtx = event->Summary->Vertex->TrueVertex->TruthVertexID; 

        //What we need to get the RooTrackerVtx is truth vertex identifier
        //which is given by AnaTrueVertexB::TrueVertexID
        Int_t TrueVertexID = event->Summary->Vertex->TrueVertex->TruthVertexID;
        ...
        for(int i = 0; i < NRooVtx; ++i){
            ...
            if(vtx->TruthVertexID == TrueVertexID) break;
            vtx = NULL;
        }
