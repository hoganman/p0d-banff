...
void PlotTunings(const std::string& weightfile, const std::string& eventfile, const std::string& outfile, const int desired_neutrino)
{
  ...
  double Enu;  t_sk->SetBranchAddress("Enu",&Enu);
  ...
  // Open file to save plots (default: Valid_tunings.root in current directory)
  TFile *fout;
  int dummy;
  char savename[100];
    ...
    for (int f=0; f<1; f++) {
      for (int e=0; e<2; e++) {
        for (int t=0; t<4; t++) {
          ...
          if (e==0) {// etrue
            hists[i][f][e][t] = new TH1D(savename, savename, 60, 0, 3);
          }
          else {// Q2
            hists[i][f][e][t] = new TH1D(savename, savename, 50, 0, 1);
          }
        }
      }
    }
  ...
  //-- TH1D* Ca5_Weights = new TH1D("Ca5_3SigmaVariation_Weight_eq_1","Ca5_3SigmaVariation_Weight_eq_1",60,0,3);
  //-- TH1D* MaNFFRes_Weights = new TH1D("MaNFFRes_3SigmaVariation_Weight_eq_1","MaNFFRes_3SigmaVariation_Weight_eq_1",60,0,3);
  //-- TH1D* BkgScl_Weights = new TH1D("BkgScl_3SigmaVariation_Weight_eq_1","BkgScl_3SigmaVariation_Weight_eq_1",60,0,3);
  //-- TH1D* Ca5_HighWeights = new TH1D("Ca5_3SigmaVariation_Weight_gt_1","Ca5_3SigmaVariation_Weight_gt_1",60,0,3);
  //-- TH1D* MaNFFRes_HighWeights = new TH1D("MaNFFRes_3SigmaVariation_Weight_gt_1","MaNFFRes_3SigmaVariation_Weight_gt_1",60,0,3);
  //-- TH1D* BkgScl_HighWeights = new TH1D("BkgScl_3SigmaVariation_Weight_gt_1","BkgScl_3SigmaVariation_Weight_gt_1",60,0,3);
  //-- TH1D* Ca5_LowWeights = new TH1D("Ca5_3SigmaVariation_Weight_lt_1","Ca5_3SigmaVariation_Weight_lt_1",60,0,3);
  //-- TH1D* MaNFFRes_LowWeights = new TH1D("MaNFFRes_3SigmaVariation_Weight_lt_1","MaNFFRes_3SigmaVariation_Weight_lt_1",60,0,3);
  //-- TH1D* BkgScl_LowWeights = new TH1D("BkgScl_3SigmaVariation_Weight_lt_1","BkgScl_3SigmaVariation_Weight_lt_1",60,0,3);
  //-- TH1D* Ca5_NegWeights = new TH1D("Ca5_3SigmaVariation_Weight_lt_0","Ca5_3SigmaVariation_Weight_lt_0",60,0,3);
  //-- TH1D* MaNFFRes_NegWeights = new TH1D("MaNFFRes_3SigmaVariation_Weight_lt_0","MaNFFRes_3SigmaVariation_Weight_lt_0",60,0,3);
  //-- TH1D* BkgScl_NegWeights = new TH1D("BkgScl_3SigmaVariation_Weight_lt_0","BkgScl_3SigmaVariation_Weight_lt_0",60,0,3);

  for(int i=0; i < 21; ++i){
    for(int j = 0; j < 7; ++j){
      sigmahists[i][j] = new TH1D(Form("%s_sigma_%d",dialnames[i], (j-3)), Form("%s_sigma_%d",dialnames[i], (j-3)), 60, 0, 3);
      truesigmahists[i][j] = new TH1D(Form("%s_trueSample_sigma_%d",dialnames[i], (j-3)), Form("%s_trueSample_sigma_%d",dialnames[i], (j-3)), 60, 0, 3);
      //-- for(int k = 0; k < 7; ++k){
      //--   if(i >=7 && i < 10) truepisigmahists[k][i-7][j] = new TH1D(Form("%s_%s_trueSample_sigma_%d",dialnames[i], modenames[k], (j-3)), Form("%s_%s_trueSample_sigma_%d",dialnames[i], modenames[k], (j-3)), 60, 0, 3);
      //-- }
    }
  }

  TH1D* protonFSIhist = new TH1D("Proton_FSI_hist","Proton_FSI_hist", 60, 0, 3);
  ...
  for (int ientry=0; ientry<t_w->GetEntries();ientry++)
  {
    ...
    for(int i = 0; i < 21; ++i){
      for(int j = 0; j < 7; ++j){
        ...
        sigmahists[i][j]->Fill(Enu, weights->At(dial));

        //-- if(i == 7 && (abs(Mode)==11)) truepisigmahists[0][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (abs(Mode)==12)) truepisigmahists[1][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (abs(Mode)==13)) truepisigmahists[2][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (abs(Mode)==31)) truepisigmahists[3][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (abs(Mode)==32)) truepisigmahists[4][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (abs(Mode)==33)) truepisigmahists[5][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (abs(Mode)==34)) truepisigmahists[6][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 7 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) > 1) Ca5_HighWeights->Fill(Enu);
        //-- if(i == 7 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) == 1) Ca5_Weights->Fill(Enu);
        //-- if(i == 7 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) < 1 && weights->At(dial)>=0) Ca5_LowWeights->Fill(Enu);
        //-- if(i == 7 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) < 0) Ca5_LowWeights->Fill(Enu);

        //-- if(i == 8 && (abs(Mode)==11)) truepisigmahists[0][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (abs(Mode)==12)) truepisigmahists[1][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (abs(Mode)==13)) truepisigmahists[2][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (abs(Mode)==31)) truepisigmahists[3][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (abs(Mode)==32)) truepisigmahists[4][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (abs(Mode)==33)) truepisigmahists[5][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (abs(Mode)==34)) truepisigmahists[6][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 8 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) > 1) MaNFFRes_HighWeights->Fill(Enu);
        //-- if(i == 8 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) == 1) MaNFFRes_Weights->Fill(Enu);
        //-- if(i == 8 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) < 1 && weights->At(dial)>=0) MaNFFRes_LowWeights->Fill(Enu);
        //-- if(i == 8 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) < 0) MaNFFRes_LowWeights->Fill(Enu);

        //-- if(i == 9 && (abs(Mode)==11)) truepisigmahists[0][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (abs(Mode)==12)) truepisigmahists[1][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (abs(Mode)==13)) truepisigmahists[2][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (abs(Mode)==31)) truepisigmahists[3][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (abs(Mode)==32)) truepisigmahists[4][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (abs(Mode)==33)) truepisigmahists[5][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (abs(Mode)==34)) truepisigmahists[6][i-7][j]->Fill(Enu, weights->At(dial)); //Pi
        //-- if(i == 9 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) > 1) BkgScl_HighWeights->Fill(Enu);
        //-- if(i == 9 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) == 1) BkgScl_Weights->Fill(Enu);
        //-- if(i == 9 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) < 1 && weights->At(dial)>=0) BkgScl_LowWeights->Fill(Enu);
        //-- if(i == 9 && (intmode == 1 || intmode == 5 || intmode == 6) && j == 0 && weights->At(dial) < 0) BkgScl_NegWeights->Fill(Enu);
        ...
        if (good_event) { truesigmahists[i][j]->Fill(Enu, weight); }
      }
    }


    protonFSIhist->Fill(Enu, weights->At(dial));
    ...
    {
      hists[0][0][0][target]->Fill(Enu, weights->At(0));
      hists[0][0][1][target]->Fill(Q2, weights->At(0));

      // rpa only
      hists[1][0][0][target]->Fill(Enu, weights->At(1));
      hists[1][0][1][target]->Fill(Q2, weights->At(1));

      // maqe only
      hists[2][0][0][target]->Fill(Enu, weights->At(2));
      hists[2][0][1][target]->Fill(Q2, weights->At(2));

      // mec_norm_c only
      hists[3][0][0][target]->Fill(Enu, weights->At(3));
      hists[3][0][1][target]->Fill(Q2, weights->At(3));

      // pf_c only
      hists[4][0][0][target]->Fill(Enu, weights->At(4));
      hists[4][0][1][target]->Fill(Q2, weights->At(4));

      // all ccqe tunings
      hists[5][0][0][target]->Fill(Enu, weights->At(5));
      hists[5][0][1][target]->Fill(Q2, weights->At(5));

      // all tunings
      hists[6][0][0][target]->Fill(Enu, weights->At(6));
      hists[6][0][1][target]->Fill(Q2, weights->At(6));

      // all tunings
      hists[8][0][0][target]->Fill(Enu, weights->At(8));
      hists[8][0][1][target]->Fill(Q2, weights->At(8));

      // untuned
      hists[7][0][0][target]->Fill(Enu);
      hists[7][0][1][target]->Fill(Q2);

      if(target != 3){
        // sf only
        hists[0][0][0][3]->Fill(Enu, weights->At(0));
        hists[0][0][1][3]->Fill(Q2, weights->At(0));

        // rpa only
        hists[1][0][0][3]->Fill(Enu, weights->At(1));
        hists[1][0][1][3]->Fill(Q2, weights->At(1));

        // maqe only
        hists[2][0][0][3]->Fill(Enu, weights->At(2));
        hists[2][0][1][3]->Fill(Q2, weights->At(2));

        // mec_norm_c only
        hists[3][0][0][3]->Fill(Enu, weights->At(3));
        hists[3][0][1][3]->Fill(Q2, weights->At(3));

        // pf_c only
        hists[4][0][0][3]->Fill(Enu, weights->At(4));
        hists[4][0][1][3]->Fill(Q2, weights->At(4));

        // all ccqe tunings
        hists[5][0][0][3]->Fill(Enu, weights->At(5));
        hists[5][0][1][3]->Fill(Q2, weights->At(5));

        // all tunings
        hists[6][0][0][3]->Fill(Enu, weights->At(6));
        hists[6][0][1][3]->Fill(Q2, weights->At(6));

        // all tunings
        hists[8][0][0][3]->Fill(Enu, weights->At(8));
        hists[8][0][1][3]->Fill(Q2, weights->At(8));

        // untuned
        hists[7][0][0][3]->Fill(Enu);
        hists[7][0][1][3]->Fill(Q2);
      }
    }
  ...
  for(int i = 0; i < 21; ++i){
    ...
    //--       sigmahists[i][j]->SetXTitle("E_{#nu} (GeV)");
    //--       sigmahists[i][j]->Draw();
    //--     }
    ...
    //--       sigmaratios[i][j]->SetXTitle("E_{#nu} (GeV)");
    ...
    std::string tname = "3 sigma histograms for true interactions ";
      ...
      for(int j = 6; j > -1; --j){
        ...
        if(j == 6){
          ...
          truesigmahists[i][j]->SetXTitle("E_{#nu} (GeV)");
    ...
    std::string tname1 = "3_sigma_ratios_true_interactions_";
      ...
      for(int j = 6; j > -1; --j){
        ...
        if(j == 6){
          ...
          truesigmaratios[i][j]->SetXTitle("E_{#nu} (GeV)");
    ...
    //--         truepisigmahists[k][i-7][j]->SetXTitle("E_{#nu} (GeV)");
    ...
    //--         truepisigmaratios[k][i-7][j]->SetXTitle("E_{#nu} (GeV)");
  ...
  for (int s=0; s<1; s++) {
    for (int e=0; e<2; e++) {
      for (int f=0; f<4; ++f) {
        ...
        if(e == 0) name.append(" vs E_{#nu}");
        if(e == 1) name.append(" vs Q^{2}");
        ...
        if(e == 0){
          hists[7][s][e][f]->SetXTitle("E_{#nu} (GeV)");
          ratios[7][s][e][f]->SetXTitle("E_{#nu} (GeV)");
        }
        else{
          hists[7][s][e][f]->SetXTitle("Q^{2} (GeV^{2})");
          ratios[7][s][e][f]->SetXTitle("Q^{2} (GeV^{2})");
        }
...
namespace {
  ...
  void Usage(){
    ...
    cout << "./Tuning_Validation_ND280_NIWG.exe -w weightfile -s skfile  [-o outputfile]" << endl;
    exit(2);
  }
