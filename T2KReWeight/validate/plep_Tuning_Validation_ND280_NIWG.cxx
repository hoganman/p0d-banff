...
// To be used for plotting Plep and Q2, instead of Enu and Q2
//
//---------------------------------------------------------------
...
void PlotTunings(const std::string& weightfile, const std::string& eventfile, const std::string& outfile, const int desired_neutrino)
{
  ...
  for(int i = 0; i < 21; ++i){
    TLegend *tleg = new TLegend(0.625, 0.525, 0.89, 0.69);
    ...
    std::string tname = "3 sigma histograms for true interactions ";
      ...
      //if(j == 6){
        if(j == 5){
          ...
          truesigmahists[i][j]->SetXTitle("P_{lep} (GeV)");
    ...
    TLegend *tleg1 = new TLegend(0.625, 0.525, 0.89, 0.69);
    ...
    std::string tname1 = "3_sigma_ratios_true_interactions_";
      ...
      for(int j = 5; j > 0; --j){
        ...
        if(j == 5){
          ...
          truesigmaratios[i][j]->SetXTitle("P_{lep} (GeV)");
          truesigmaratios[i][j]->SetMinimum(0.5);
          truesigmaratios[i][j]->SetMaximum(1.5);
