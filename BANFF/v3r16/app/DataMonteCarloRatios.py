...
for i in xrange(0,len(sampleNames)):
    ...
    for ibx in xrange(1, residuals.GetNbinsX() + 1):
        ...
        for iby in xrange(1, residuals.GetNbinsY() + 1):
            ...
            residuals.SetBinContent(ibx,iby,(pC-dC)/dErr)
            prefitResiduals.SetBinContent(ibx,iby,(preC-dC)/preErr)
    ...
    cPre.cd(4)
    prefitResiduals.SetTitle(sampleNames[i] + ": Residuals (prefit - data)/sqrt(prefit)")
    ...
    cPost.cd(4)
    residuals.SetTitle(sampleNames[i] + ": Residuals (postfit - data)/sqrt(postfit)")
