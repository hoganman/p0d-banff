...
int main(int argc, char** argv){
    ...
    //Go through the fit parameters up to this point, and if they are CCQE
    //parameters, set their vary flag to false.
    //Since we know what the configuration is, just do this by parameter
    //number, and print out warnings about what we're doing in case somebody
    //blindly copies and pastes this...
    //Only do this for MAQE, pF_C, and MEC_C (106, 107, 108)
    std::cout << "===========================================" << std::endl;
    for(int fi = 106; fi < 109; fi++){

        fitParameters->SetDoVary(fi,false);
        fitParameters->SetThrowParameter(fi,false);
        std::cout << fitParameters->GetParamName(fi) << " vary and throwParms are now set to " << fitParameters->GetDoVary(fi) << std::endl;
