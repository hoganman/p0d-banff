...
void PlotCorrelationMatrix_P0DOnly(std::string inputFileName = "")
{
    gStyle->SetOptStat(0);
    ...
    {
        printf("USAGE: root PlotCorrelationMatrix(\"INPUTFILENAME\")\n");
        return;
    }
    ...
    if(!params || !postfit_cov || !prefit_cov)
    {
        printf("Unable to find either params, postfit_cov, or prefit_cov, returning...\n");
        return;
    }

    const int n_xsec  =  31;
    const int n_flux  = 100;
    const int n_dete  = 144;
    const int n_total = n_xsec + n_flux + n_dete;
    ...
    cov_pref.GetZaxis()->SetRangeUser(-0.01, 0.10);
    ...
    cov_dete.GetXaxis()->SetRangeUser(n_flux+n_dete,n_total);
    cov_dete.GetYaxis()->SetRangeUser(n_flux+n_dete,n_total);
    cov_dete.GetZaxis()->SetRangeUser(-0.015, 0.015);
    ...
    corr_dete.GetXaxis()->SetRangeUser(n_flux+n_dete,n_total);
    corr_dete.GetYaxis()->SetRangeUser(n_flux+n_dete,n_total);
	...
	cov_xsec.GetZaxis()->SetRangeUser(-1.5, 1.5);
	cov_xsec.Draw("COLZ");
