...
int main(int argc, char** argv){
    ...
    if(argc < 9){
        ...
        std::cout << "Usage: Make2015PostFitFileBias.exe preFitFile.root FittedParameters0 FittedCovariance0 flux_uncertainty_file.root FittedParameters0 FittedCovariance0 biasfile.root  outputfile.root" << std::endl;
        return 1;
    }
    ...
    //Load the bias covariance file in
    TFile* biasFile = new TFile(argv[8]);
    TMatrixDSym* biasCov = (TMatrixDSym*)(biasFile->Get("bias_cov"));
    TVectorD* biasVec = (TVectorD*)(biasFile->Get("bias_frac"));
    ...
    TFile* outputFile = new TFile(argv[9],"RECREATE");
    ...
    for(int i = 0; i < preFitParams->GetNrows(); i++){
	    ...
	    (*outputPostFitParams_temp)(spi) = (*outputPostFitParams_temp)(spi) + (*biasVec)(spi);
            //Save its name.
            ...
            for(int j = 0; j < preFitParams->GetNrows(); j++){
                ...
                if(saveThisParam[j]){
                    ...
                    (*outputPostFitCov_temp)(spi,spj) = (*postFitCov)(i,j) + (*biasCov)(spi,spj) ;
    ...
    NC1GAMMA_bins->Write("NC1GAMMA_bins");
