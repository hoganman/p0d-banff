...
dummyPage = TCanvas()
dummyPage.Print("reactionCodeStacksOfficial_Joint2016_numode_postfit_mom.pdf[")
#gStyle.SetErrorX(0.0000001)

#Set up the array of sample names.
#This version is just for neutrino mode samples.
#sampleNames = ["cc0pi","cc1pi","ccOth","anuccqe","anuccnqe","nuccqe","nuccnqe"]
#fileNameRoots = ["cc0pi","cc1pi","ccOth","acc1tr","accntr","cc1tr","ccntr"]

#sampleNames = ["cc0pi","cc1pi","ccOth"]
#fileNameRoots = ["cc0pi","cc1pi","ccOth"]
sampleNames = ["fgd1cc0pi","fgd1cc1pi","fgd1ccOth","fgd2cc0pi","fgd2cc1pi","fgd2ccOth"]
fileNameRoots =  ["fgd1cc0pi","fgd1cc1pi","fgd1ccOth","fgd2cc0pi","fgd2cc1pi","fgd2ccOth"]
...
stackHistos = ["#nu CCQE","#nu CC 2p-2h", "#nu CC Res 1#pi", "#nu CC Coh 1#pi",
"#nu CC Other", "#nu NC modes"
,"#bar{#nu} modes"]


latexTitles = ["#splitline{FGD 1 #nu_{#mu}}{CC-0#pi (PF)}","#splitline{FGD 1 #nu_{#mu}}{CC-1#pi (PF)}","#splitline{FGD 1 #nu_{#mu}}{CC-Other (PF)}","#splitline{FGD 2 #nu_{#mu}}{CC-0#pi (PF)}","#splitline{FGD 2 #nu_{#mu}}{CC-1#pi (PF)}","#splitline{FGD 2 #nu_{#mu}}{CC-Other (PF)}"]
...
#Picking and choosing from the 12 colour palette.
stackColours = [1300,
        1302, 1304, 1305, 1307, 1308,
        1310]

#Using the 7 colour medium palette.
stackColours = [1400,1401,1402,1403,1404,1405,1406]

#Choosing differently from the 12 colour palette.
#stackColours = [1300,
#        1302, 1304, 1305, 1308, 1310,
#        1311]

stackColours = [1300,1302,1303,1304,1305,1308,1310]

#stackColours = [kBlue, kMagenta, kRed, 1300, 1303, kGray + 1,
#        kBlue, kMagenta, kRed, kCyan, kOrange+1, kGray+1]
...
#stackColours = [2,4,6,7,30,11,12,28,
#        2,4,6,7,8,11,12,28]
...
stackFillStyle = [1001,1001,1001,1001,1001,1001,
        1001]
...
for ii in xrange(0,len(sampleNames)):
    ...
    for ibx in xrange(1, dataMomProj.GetNbinsX() + 1):
        ...
        print "Data = "+str(dataMomProj.GetBinContent(ibx))+"  MC = "+str(postfitMomProj.GetBinContent(ibx))+" Ratio = "+str(dataMomProj.GetBinContent(ibx)/postfitMomProj.GetBinContent(ibx))
    ...
    dataMomProj.GetXaxis().SetRangeUser(0,5000)
...
#        prelim.Draw("")
        ...
        c1.Modified()
        c1.Update()
        c1.Print("reactionCodeStacksOfficial_Joint2016_numode_postfit_mom.pdf")
        outFileName = fileNameRoots[sampleNames.index(sampleName)] + "_mumom_rxn_postfit"
