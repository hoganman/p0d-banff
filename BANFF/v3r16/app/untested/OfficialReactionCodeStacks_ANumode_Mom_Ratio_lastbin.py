...
dummyPage = TCanvas()
dummyPage.Print("reactionCodeStacksOfficial_Joint2016_anumode_postfit_mom.pdf[")
...
for ii in xrange(0,len(sampleNames)):
    ...
    dataMomProj = dataFile.Get(sampleName + "_data").Projection(1,0).ProjectionX("dataratio" + sampleName + "_px",1,dataHist.GetNbinsY()) 
    dataMomProj.SetDirectory(0)
    dataMomProj.SetTitle("")
   
    prefitMomProj = dataFile.Get(sampleName + "_prefit").Projection(1,0).ProjectionX("prefitratio" + sampleName + "_px",1,dataHist.GetNbinsY())
    prefitMomProj.SetDirectory(0)

    postfitMomProj = dataFile.Get(sampleName + "_postfit_0_0").Projection(1,0).ProjectionX("postfitratio" + sampleName + "_px",1,dataHist.GetNbinsY())
    postfitMomProj.SetDirectory(0)

    for ibx in xrange(1, dataMomProj.GetNbinsX() + 1):
        #Do the momentum projections here:
        dataMomProj.SetBinError(ibx,ROOT.Math.sqrt(dataMomProj.GetBinContent(ibx))/postfitMomProj.GetBinContent(ibx))
        ...
        dataMomProj.SetBinContent(ibx,dataMomProj.GetBinContent(ibx)/postfitMomProj.GetBinContent(ibx))

    dataMomProj.GetYaxis().SetRangeUser(0.75,1.25)
    dataMomProj.GetXaxis().SetRangeUser(0,6000)
...
#                        histosToStack[i].GetBinContent(ibx,iby)/mpdim)
    ...
    for odi in xrange(0, len(oneDimPlots[0])):
        ...
        nx0 = dataOneDimPlots[odi].GetXaxis().GetNbins()
        x0 = dataOneDimPlots[odi].GetXaxis().GetXbins()
        x0.SetAt(6000,nx0)
        h0 = TH1D("hist_cs_2",dataOneDimPlots[odi].GetTitle(),nx0,x0.GetArray())
        for i in range(1,nx0+1):
            h0.SetBinContent(i,dataOneDimPlots[odi].GetBinContent(i))  
        h0.SetEntries(dataOneDimPlots[odi].GetEntries())
        ...
        for odj in xrange(0, len(oneDimPlots)):
            nx1 = oneDimPlots[odj][odi].GetXaxis().GetNbins()
            x1 = oneDimPlots[odj][odi].GetXaxis().GetXbins()
            x1.SetAt(6000,nx1)
            h1 = TH1D("hist_cs_2",oneDimPlots[odj][odi].GetTitle(),nx1,x1.GetArray())
            for i in range(1,nx1+1):
                h1.SetBinContent(i,oneDimPlots[odj][odi].GetBinContent(i))  
            h1.SetEntries(oneDimPlots[odj][odi].GetEntries())
            
            
            #Apply the appropriate plot options from the beginning.
            h1.SetFillColor(stackColours[odj])
            h1.SetFillStyle(stackFillStyle[odj])
            h1.SetMarkerStyle(1) #Don't want T2KStyle to interfere.
            h1.GetXaxis().SetRangeUser(0,6000)
            sampleStack.Add(h1)
            ...
            if true:
                ...
                leg.AddEntry(h1,stackHistos[odj],"F")
...
#        lowerPad.SetFrameFillStyle(4000)
        ...
        nx = dataMomProj.GetXaxis().GetNbins()
        x = dataMomProj.GetXaxis().GetXbins()
        x.SetAt(6000,nx)
        h2 = TH1D("hist_cs_2",dataMomProj.GetTitle(),nx,x.GetArray())
        for i in range(1,nx+1):
            h2.SetBinContent(i,dataMomProj.GetBinContent(i))  
            h2.SetBinError(i,dataMomProj.GetBinError(i))  
        h2.SetEntries(dataMomProj.GetEntries())
        ...
        h2.SetLineColor(2) 
        h2.SetMarkerColor(2) 
        h2.GetYaxis().SetNdivisions(8,1)
        h2.GetYaxis().SetLabelSize(h2.GetYaxis().GetLabelSize()+0.07)
        h2.GetYaxis().SetTitleSize(h2.GetYaxis().GetLabelSize()+0.05)
        h2.GetYaxis().SetTitleOffset(0.335)
        h2.GetXaxis().SetLabelSize(h2.GetXaxis().GetLabelSize()+0.08)
        h2.GetXaxis().SetTitleSize(h2.GetXaxis().GetLabelSize()+0.04)
        h2.GetXaxis().SetTitle("Reconstructed muon momentum (MeV/c)")
        h2.GetXaxis().SetTitleOffset(0.9)
        h2.GetXaxis().SetTickLength(0.04)
        h2.GetYaxis().SetRangeUser(0.75,1.25)
        h2.GetXaxis().SetNdivisions(0,0)
        h2.GetXaxis().SetLabelOffset(99)
        h2.Draw("PE") 
        line = TLine(0.0,1.0,6000,1.0)
        line.SetLineWidth(2)
        line.SetLineColor(1)
        line.Draw("SAME")
        ...
        if ii%4 == 0:
            labels = ["0","1000","2000","10000"]
            xposition = [0.105,0.23,0.372,0.93]
            y = 0.24
            t =  TText()
            t.SetTextSize(0.12)
            for i in range(4):
                t.DrawTextNDC(xposition[i],y,labels[i])
            xposition = [0.25,0.392]
            y1 = 0.35
            y2 = 0.95
            t =  TText()
            t.SetTextSize(0.1)
            for i in range(2):
                t.DrawTextNDC(xposition[i],y1,"|")
                t.DrawTextNDC(xposition[i],y2,"|")
        elif ii%4 == 1:
            labels = ["0","1000","2000","3000","10000"]
            xposition = [0.105,0.23,0.372,0.515,0.93]
            y = 0.24
            t =  TText()
            t.SetTextSize(0.12)
            for i in range(5):
                t.DrawTextNDC(xposition[i],y,labels[i])
            xposition = [0.25,0.392,0.532]
            y1 = 0.35
            y2 = 0.95
            t =  TText()
            t.SetTextSize(0.1)
...
#        prelim.Draw("")
        ...
        c1.Modified()
        c1.Update()
        c1.Print("reactionCodeStacksOfficial_Joint2016_anumode_postfit_mom.pdf")
        outFileName = fileNameRoots[sampleNames.index(sampleName)] + "_mumom_rxn_prefit"
