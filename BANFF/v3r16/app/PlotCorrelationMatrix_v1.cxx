...
void PlotCorrelationMatrix_v1() {
	gStyle->SetOptStat(0);
	gStyle->SetPaintTextFormat("4.2f");
	gStyle->SetTextFont(40);
	gROOT->SetBatch(kTRUE);

    TFile *inputFile = new TFile("postfit_v1_DET_MCSTATS_1p1h.root");
	...
	string binLabels[31];
	for (int i = n_flux; i < n_flux+n_xsec; i++){
		binLabels[i-n_flux] = (((TObjString*)(params->At(i)))->GetString()).Data();
		//std::cout<<binLabels[i-n_flux]<<std::endl;
	}
	...
	for(int i = n_flux; i < n_flux+n_xsec; i++){
		for(int j = n_flux; j < n_flux+n_xsec; j++){
            cov_xsec.SetBinContent(cov_xsec.GetBin(i-n_flux+1,j-n_flux+1), (*postfit_cov)(i,j));
            correlation = (*postfit_cov)(i,j) / sqrt((*postfit_cov)(i,i) * (*postfit_cov)(j,j));
			if (fabs(correlation) > 0.01) { corr_xsec.SetBinContent(corr_xsec.GetBin(i-n_flux+1,j-n_flux+1), correlation); }
		}
		corr_xsec.GetXaxis()->SetBinLabel(corr_xsec.GetBin(i-n_flux+1), binLabels[i-n_flux].c_str());
		corr_xsec.GetYaxis()->SetBinLabel(corr_xsec.GetBin(i-n_flux+1), binLabels[i-n_flux].c_str());
		cov_xsec.GetXaxis()->SetBinLabel(cov_xsec.GetBin(i-n_flux+1), binLabels[i-n_flux].c_str());
		cov_xsec.GetYaxis()->SetBinLabel(cov_xsec.GetBin(i-n_flux+1), binLabels[i-n_flux].c_str());
	}
    std::cout << "Finished xsec" <<std::endl;

    // Detector correlation matrix
	for(int i = n_flux+n_xsec; i < n_total; i++){
		for(int j = n_flux+n_xsec; j < n_total; j++){
            cov_dete.SetBinContent(cov_dete.GetBin(i-n_flux-n_xsec+1,j-n_flux-n_xsec+1), (*postfit_cov)(i,j));
            correlation = (*postfit_cov)(i,j) / sqrt((*postfit_cov)(i,i) * (*postfit_cov)(j,j));
			if (fabs(correlation) > 0.01) { corr_dete.SetBinContent(corr_dete.GetBin(i-n_flux-n_xsec+1,j-n_flux-n_xsec+1), correlation); }
		}
	}
    ...
    dummy.Print("v1_correlation_matrices.pdf[");
    ...
    cov_pref.Draw("COLZ");
    c0.Print("v1_correlation_matrices.pdf");
    ...
    corr_pref.Draw("COLZ");
    c1.Print("v1_correlation_matrices.pdf");
	...
	cov_post.GetYaxis()->SetTitle("Parameter Number");
  	cov_post.Draw("COLZ");
	c2.Print("v1_correlation_matrices.pdf");
	...
	corr_post.GetYaxis()->SetTitle("Parameter Number");
  	corr_post.Draw("COLZ");
	c3.Print("v1_correlation_matrices.pdf");
    ...
    cov_flux.Draw("COLZ");
    c4.Print("v1_correlation_matrices.pdf");
    ...
    corr_flux.Draw("COLZ");
    c5.Print("v1_correlation_matrices.pdf");
    ...
    cov_dete.Draw("COLZ");
    c6.Print("v1_correlation_matrices.pdf");
    ...
    corr_dete.Draw("COLZ");
    c7.Print("v1_correlation_matrices.pdf");
	...
	c8.Print("v1_correlation_matrices.pdf");
	...
	c9.Print("v1_correlation_matrices.pdf");
