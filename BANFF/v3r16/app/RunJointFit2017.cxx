...
int main(int argc, char** argv){
    ...
    if(ND::params().GetParameterI("BANFF.SavePrefitParamsAndExit")){
      ...
      if(!ND::params().GetParameterI("BANFF.RunFit.FitData") && !ND::params().GetParameterI("BANFF.RunFit.FitNominalMC")){
	...
	//Need to copy prior values across to current value to be written out
    ...
    //Now that all parameters are added, flag the flux, detector (obsnorm) and FSI parameters for
    //decomposition.
	...
	if(ND::params().GetParameterI("BANFF.DoLikelihoodScan")) {
