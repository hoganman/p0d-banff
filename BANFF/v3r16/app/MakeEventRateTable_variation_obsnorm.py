...
#Usage: MakeEventRateTable_variation_obsnorm.py inputFile outputFile
from ROOT import *
import sys
import copy
import math
...
obsinfile = TFile('/mnt/home/morri631/BANFF_Summer2018_Analysis/software/BANFF/v3r16/inputs/obsNorm_2018a_v1.root')
obs_cov   = obsinfile.Get("obsNorm_cov")

sigma = []

for i in xrange(0,obs_cov.GetNrows()):
    sigma.append(sqrt(obs_cov(i,i)))
...
for line in lines:
    ...
    if words[0] not in dontwant:
        values_mach3_tmp.append(words[0])
        #print words[0]

# 100 flux parameters in 14 samples with 3 values each (-1, 0, +1 sigma)
nFluxVals = 3 * 14 * 100 
values_mach3 = values_mach3_tmp[nFluxVals:]
...
for i in xrange(0 ,obs_cov.GetNrows()):
    paramNameList.append("OBSNORM_"+str(i))
