...
int main(int argc, char** argv){
    ...
    //Need to skip both MEC parameters now, since treating them as
    //normalizations, which should not have 1 subtracted from them at this
    //point.  They are at index 2 and 5.
    for(int i = 0; i < 11; i++){


        //If it's a MEC index, skip this part.
        if((i == 2) || (i == 5)){
            continue;
        }
    ...
    //Easiest thing to do is to remove MEC_O and rename at the last minute.
    //At this point, MEC_C is at position 8, and MEC_O is at position 11
    //Start by making the relevant changes for MEC_C to MEC_ND280.
    xsec_param_namesFSIFirst->AddAt(new TObjString("MEC_ND280"), 8);
    xsec_param_namesFSIFirst->AddAt(new TObjString("MEC_SK"),11);

    //Restrict MEC_SK to only target material 999, which does not exist.
    (*xsec_param_targetFSIFirst)(11) = 999.0;

    //Make a target list for MEC_ND280, setting it to 12.0 and 16.0.
    TVectorD* MEC_ND280_targets = new TVectorD(2);
    (*MEC_ND280_targets)(0) = 12.0;
    (*MEC_ND280_targets)(1) = 16.0;


    //For test purposes.
    //(*xsec_param_targetFSIFirst)(8) = 12.0;
    //(*xsec_param_targetFSIFirst)(11) = 16.0;

    //Another test.  Set to Radon, which there should be precious little of, to
    //be sure we're not picking up some fringe case with 999.0.
    //(*xsec_param_targetFSIFirst)(11) = 222.0;

    //With that name, it will automatically be set as a NORM parameter, and
    //poll the axes which are common for all XSEC NORM parameters.  The limits
    //are already there, so we just need to set the reaction codes (2 and -2)
    //and we're set.
    TVectorD* MEC_ND280_reactionCodes = new TVectorD(2);
    (*MEC_ND280_reactionCodes)(0) = 2.0;
    (*MEC_ND280_reactionCodes)(1) = -2.0;

    //This parameter never gets considered, but include this for completeness.
    TVectorD* MEC_SK_reactionCodes = new TVectorD(2);
    (*MEC_SK_reactionCodes)(0) = 2.0;
    (*MEC_SK_reactionCodes)(1) = -2.0;
    
    
//    //--------------------------------------
//    //The easiest way to implement putting FSI first is to just create new
//    //versions of everything that are one smaller, and neglect to copy MEC_O
//    TMatrixDSym* xsec_covFSIFirstNoMECO = new TMatrixDSym(nParams-1);
//    TVectorD* xsec_param_priorFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_nomFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_lbFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_ubFSIFirstNoMECO = new TVectorD(nParams-1);
//    TObjArray* xsec_param_namesFSIFirstNoMECO = new TObjArray(nParams-1);
//    TVectorD* xsec_param_throwFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_fitFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_constrainFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_splineFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_targetFSIFirstNoMECO = new TVectorD(nParams-1);
//    TVectorD* xsec_param_binFSIFirstNoMECO =  new TVectorD(nParams-1);
//    TObjArray* xsec_param_binsnameFSIFirstNoMECO = new TObjArray(nParams-1);
//    TVectorD* xsec_param_discreteFSIFirstNoMECO = new TVectorD(nParams-1);
//
//    //Loop over the full matrix.
//    //Move indices 16 to 21 to 0-5, move 0-15 to 6-21, and keep 22-23 where
//    //they are.
...
//        int newi;
//
//        //Pre-MEC_O, keep same index number
//        if(oldi < 11){
//
//            newi = oldi;
//        }
//
//        //Pos-MEC_O, subtract one index.
//        else if(oldi > 11){
//
//            newi = oldi - 1;
//
//        }
//
//        //Otherwise it is MEC_O, so just ignore.
//        else{
//
//            continue;
//        }
//
//        (*xsec_param_priorFSIFirstNoMECO)(newi) = (*xsec_param_priorFSIFirst)(oldi);
//        (*xsec_param_nomFSIFirstNoMECO)(newi) = (*xsec_param_nomFSIFirst)(oldi);
//        (*xsec_param_lbFSIFirstNoMECO)(newi) = (*xsec_param_lbFSIFirst)(oldi);
//        (*xsec_param_ubFSIFirstNoMECO)(newi) = (*xsec_param_ubFSIFirst)(oldi);
//        xsec_param_namesFSIFirstNoMECO->AddAt(xsec_param_namesFSIFirst->At(oldi),newi);
//        (*xsec_param_throwFSIFirstNoMECO)(newi) = (*xsec_param_throwFSIFirst)(oldi);
//        (*xsec_param_fitFSIFirstNoMECO)(newi) = (*xsec_param_fitFSIFirst)(oldi);
//        (*xsec_param_constrainFSIFirstNoMECO)(newi) = (*xsec_param_constrainFSIFirst)(oldi);
//        (*xsec_param_splineFSIFirstNoMECO)(newi) = (*xsec_param_splineFSIFirst)(oldi);
//        (*xsec_param_targetFSIFirstNoMECO)(newi) = (*xsec_param_targetFSIFirst)(oldi);
//        (*xsec_param_binFSIFirstNoMECO)(newi) = (*xsec_param_binFSIFirst)(oldi);
//        xsec_param_binsnameFSIFirstNoMECO->AddAt(xsec_param_binsnameFSIFirst->At(oldi),newi);
//        (*xsec_param_discreteFSIFirstNoMECO)(newi) = (*xsec_param_discreteFSIFirst)(oldi);
//
//        //With the parameters taken care of, now take care of the covariance
//        //matrix.
//        for(int oldj = 0; oldj < nParams; oldj++){
...
//
//
//            (*xsec_covFSIFirstNoMECO)(newi,newj) = (*xsec_covFSIFirst)(oldi,oldj);
//
//        }
//    }
//--------------------------------------
    ...
    //With FSI first.
//    xsec_covFSIFirstNoMECO->Write("xsec_cov");
//    xsec_param_priorFSIFirstNoMECO->Write("xsec_param_prior");
//    xsec_param_nomFSIFirstNoMECO->Write("xsec_param_nom");
//    xsec_param_lbFSIFirstNoMECO->Write("xsec_param_lb");
//    xsec_param_ubFSIFirstNoMECO->Write("xsec_param_ub");
//    xsec_param_namesFSIFirstNoMECO->Write("xsec_param_names",TObject::kSingleKey);
//    xsec_param_throwFSIFirstNoMECO->Write("xsec_param_throw");
//    xsec_param_fitFSIFirstNoMECO->Write("xsec_param_fit");
//    xsec_param_constrainFSIFirstNoMECO->Write("xsec_param_constrain");
//    xsec_param_splineFSIFirstNoMECO->Write("xsec_param_spline");
//    xsec_param_targetFSIFirstNoMECO->Write("xsec_param_target");
//    xsec_param_binFSIFirstNoMECO->Write("xsec_param_bin");
//    xsec_param_binsnameFSIFirstNoMECO->Write("xsec_param_binsname", TObject::kSingleKey);
//    xsec_param_discreteFSIFirstNoMECO->Write("xsec_param_discrete");
    ...
    MEC_ND280_reactionCodes->Write("MEC_ND280_reactionCodes");
    MEC_SK_reactionCodes->Write("MEC_SK_reactionCodes");
    ...
    MEC_ND280_targets->Write("MEC_ND280_targets");
