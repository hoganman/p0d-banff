...
int main(int argc, char** argv){
    ...
    for(int ti = 0; ti < nToys; ti++){
        ...
        for(int si1 = 0; si1 < nSamples; si1++){
            ...
            int* bins1 = BANFFBinnedSampleUtils::GetBins(tmp1);
            for(int bi1 = 0; bi1 < BANFFBinnedSampleUtils::GetNBins(tmp1); bi1++){

                double delta1 = (1.0 - (tmp1->GetBinContent(bins1[bi1])/mean[si1]->GetBinContent(bins1[bi1])));
                ...
                for(int si2 = 0; si2 < nSamples; si2++){
                    ...
                    int* bins2 = BANFFBinnedSampleUtils::GetBins(tmp2);
                    for(int bi2 = 0; bi2 < BANFFBinnedSampleUtils::GetNBins(tmp2); bi2++){


                        double delta2 = (1.0 - (tmp2->GetBinContent(bins2[bi2])/mean[si2]->GetBinContent(bins2[bi2])));
                    ...
                    //We're done with the contents of tmp2 for now, so delete
                    //it from memory.
                    delete tmp2;
            ...
            //We're done with the contents of tmp2, so delete it from memory.
            delete tmp1;
