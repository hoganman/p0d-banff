...
Minuit2LikelihoodFit::Minuit2LikelihoodFit(BANFFInterfaceBase* inter, std::string outputFileName){
  ...
  //Print out the number of events applied to each parameter
  std::cout << "======================================" << std::endl;
  std::cout << "Events applied to each parameter:" << std::endl;
  std::cout << "--------------------------------------" << std::endl;
  for(int i=0; i < interface->fitParameters->GetNumbSysts(); i++){
    std::cout << interface->fitParameters->GetParamName(i) <<": " << interface->fitParameters->GetNEventsAppliedTo(i) << std::endl;
  }
  std::cout << "======================================" << std::endl;
...
void Minuit2LikelihoodFit::InitializeParameters(){
  ...
  for(int i=0; i < interface->fitParameters->GetNumbSysts(); i++){
    ...
    std::cout << interface->fitParameters->GetParamName(i) << " ("
	      ...
	      << interface->fitParameters->GetCurrentValue(i) << " +/- "
...
void Minuit2LikelihoodFit::InitializeParametersFromFile(std::string inputFileName){
  ...
  TVectorT<double>* inputParams = (TVectorT<double>*)inputFile->Get("FittedParameters0");
		
  std::cout << "======================================" << std::endl;
  std::cout << "Initialize parameters from file : " <<inputFileName.c_str()<< std::endl;
  std::cout << "Please make sure that the parameters used, and the parameter order are the same !"<< std::endl;
  for(int i=0; i < interface->fitParameters->GetNumbSysts(); i++){
    ...
    interface->fitParameters->SetParameterValue(i,(*inputParams)(i));


    std::cout << interface->fitParameters->GetParamName(i) <<" "<< interface->fitParameters->GetCurrentValue(i,true)<<std::endl;
    ...
    //them as well.
    if( (!(interface->fitParameters->GetDoVary(i))) || (interface->fitParameters->GetIsDiscrete(i)) ){
      //std::cout<<"Fixed parameter : "<<interface->fitParameters->GetParamName(i)<<std::endl;
      fminuit->FixParameter(i);
    }
...
void Minuit2LikelihoodFit::ScanParameters(){
  ...
  std::vector<double> pars = params->Params();
  TGraph** par_scans_penalty = new TGraph*[pars.size()];
  TGraph** par_scans_sample  = new TGraph*[pars.size()];
  TGraph** par_scans_total   = new TGraph*[pars.size()];

  for (int i=0; i<pars.size(); ++i) {
      par_scans_penalty[i] = NULL;
      par_scans_sample [i] = NULL;
      par_scans_total  [i] = NULL;
  }
  ...
  for(int i=0; i < interface->fitParameters->GetNumbSysts(); i++){
    ...
    if( (interface->fitParameters->GetDoVary(i)) || (!(interface->fitParameters->GetIsDiscrete(i)))){
      ...
      double min = pars[ii]-5.0*10*3*params->Error(ii);
      double max = pars[ii]+5.0*10*3*params->Error(ii);
	  
      ROOT::Minuit2::MnScan mnscan_nd280(*minuit2_likelihood_fit_fcn, *params);
      std::cout << "SCANNING: " << interface->fitParameters->GetParamName(i);
      std::cout << "   From " << min;
      std::cout << "   To   " << max << std::endl;

      par_scans_sample[ii] = new TGraph();
      interface->OnlySampleContrib  = true;
      interface->OnlyPenaltyContrib = false;
      par_scans_sample[ii]->SetName(Form("%s_scan_sample",interface->fitParameters->GetParamName(i).c_str()));
      std::vector< std::pair<double,double> > pscan_sample = mnscan_nd280.Scan(i, 50, min, max);

      for(unsigned int j=1; j<pscan_sample.size(); j++){
        par_scans_sample[ii]->SetPoint(j-1,pscan_sample[j].first,pscan_sample[j].second);
      }
	    
      par_scans_penalty[ii] = new TGraph();
      interface->OnlySampleContrib  = false;
      interface->OnlyPenaltyContrib = true;
      par_scans_penalty[ii]->SetName(Form("%s_scan_penalty",interface->fitParameters->GetParamName(i).c_str()));
      std::vector< std::pair<double,double> > pscan_penalty = mnscan_nd280.Scan(i, 50, min, max);

      for(unsigned int j=1; j<pscan_penalty.size(); j++){
        par_scans_penalty[ii]->SetPoint(j-1,pscan_penalty[j].first,pscan_penalty[j].second);
      }
	    
      par_scans_total[ii] = new TGraph();
      interface->OnlySampleContrib  = false;
      interface->OnlyPenaltyContrib = false;
      par_scans_total[ii]->SetName(Form("%s_scan_total",interface->fitParameters->GetParamName(i).c_str()));
      std::vector< std::pair<double,double> > pscan_total = mnscan_nd280.Scan(i, 50, min, max);

      for(unsigned int j=1; j<pscan_total.size(); j++){
        par_scans_total[ii]->SetPoint(j-1,pscan_total[j].first,pscan_total[j].second);
      }
      ii++;
    }
  }
  ...
  for(unsigned int i=0; i<pars.size(); i++){
      if (par_scans_sample [i] != NULL) { par_scans_sample [i]->Write(); }
      if (par_scans_penalty[i] != NULL) { par_scans_penalty[i]->Write(); }
      if (par_scans_total  [i] != NULL) { par_scans_total  [i]->Write(); }
  }
    
  outputFile->Close();
    
  for(unsigned int i=0; i<pars.size(); i++){
    delete par_scans_sample [i];
    delete par_scans_penalty[i];
    delete par_scans_total  [i];
  }

  delete[] par_scans_sample ;
  delete[] par_scans_penalty;
  delete[] par_scans_total  ;
}
...
void Minuit2LikelihoodFit::DoFit(){
  ...
  if(true){
    ...
    //TFitterMinuit
    ROOT::Minuit2::MnUserParameterState hesseOutput = hesse(*fcn,fitState,5000000);
  ...
  if(interface->fitParameters->eigenDecompositionRequired){
    ...
    std::cout << "Minuit2LikelihoodFit::DoFit before UnDecomposeParameters" << std::endl;
    std::cout << "-------------------------------------------------------------------------------------------" << std::endl;
    for(int i=0; i < interface->fitParameters->GetNumbSysts(); i++){
      std::cout << interface->fitParameters->GetParamName(i) << " ("
                << interface->fitParameters->GetShortName(i) << ") : "
    ...
    //Once the fit is complete and the parameters un-decomposed, print their
    //values to the terminal for easy readback.
    std::cout << "Minuit2LikelihoodFit::DoFit after UnDecomposeParameters" << std::endl;
    std::cout << "-------------------------------------------------------------------------------------------" << std::endl;
    for(int i=0; i < interface->fitParameters->GetNumbSysts(); i++){
      std::cout << interface->fitParameters->GetParamName(i) << " ("
                << interface->fitParameters->GetShortName(i) << ") : "
