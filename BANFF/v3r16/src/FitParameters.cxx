...
void XSecNormParameter::MatchToEvent(BANFFEventBase* event){
    ...
    ///Only set it to something else if it hasn't already been set.
	//Not the case anymore because of 2p2h_norm_CtoO where you have two norm parameters 
    //if(event->GetXsecNormParameter() == -1){
...
void FitParameters::AddFluxParameters(TAxis *bins, int det, int flav, int mode, bool vary){
    ...
    for(int i=0; i<bins->GetNbins(); i++){
        ...
        tmp->lowBoundScan = -15.0;
        tmp->upBoundScan = 15.0;
    ...
    //Otherwise, assume the 2016 format here.
        ...
        for(int i = 0; i < xsec_param_nom->GetNrows(); i++){
		  ...
		  std::cout<<"Adding the parameter "<< paramName << " to the list" << std::endl;
                    ...
                    AddXsecNormParameter2016(paramName, (*xsec_param_prior)(i), reacCodesVec,nupdgVec, nTgtMat, tgtMatArray, param_bins, (int)((*xsec_param_bin)(i)+0.01), throw_param);
...
void FitParameters::AddXsecNormParameter2016(std::string nameBase, double value, TVectorD* reacCodes, TVectorD* nuPDG, int nTgtMat, int* tgtMatArray, TAxis *norm_bins, int bin, bool vary){
...
void FitParameters::AddObsNormParameter(double value, std::vector<double> obsMin, std::vector<double> obsMax, int det, int sample, bool vary ){
    ...
    //lower bound set to -10.0 for allowing decomposition.
    tmp->lowBound = -10.0;
    tmp->upBound = 10.0;
    //tmp.physicsCode = -999.;
    //tmp.xsecID = NORM;
