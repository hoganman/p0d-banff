...
BANFFEventBase::BANFFEventBase(int numbSysts, int number, bool conserveMemory){
    ...
    if(!conserveMemory){
        ...
        twkDials = new float[numbXsecSysts];
...
void BANFFEventBase::SaveTotalWeight(FitParameters* fitParameters, double Q2, int NuFlavor){
    ...
    totalWeight = fluxWeight[sample->sampleID] * POTWeight;
    ...
    for(int i=0; i<fitParameters->GetNumbSysts(); i++){
        ...
        if( xsecSplines[fitParameters->GetTypeIndex(i)][sample->sampleID]!=NULL ){
			//if(debug==0) std::cout << "Spline: " << fitParameters->GetParamName(i) << " "<< xsecSplines[fitParameters->GetTypeIndex(i)][sample->sampleID]->Eval(fitParameters->GetCurrentValue(i,true))<<std::endl;
            tmp = xsecSplines[fitParameters->GetTypeIndex(i)][sample->sampleID]->Eval(fitParameters->GetCurrentValue(i,true));
    ...
    //TODO My bad I've promised not to hard code anything anymore but for now BeRPA being the only functionnal parameter it's completely hardcoded. 
	//Will find a way to automatize this (eventually)
	if(xsecFuncParameter>=0){
		//This is the hardcoded bit, BeRPA_A is parameter 117 and then the others are 118 and so on
		int BeRPA = 114;
...
void BANFFEventBase::SetFluxWeight(Double_t* weight){
    ...
    fluxWeight = new Double_t[nSamples];

    for(int i = 0; i < nSamples; i++){
      fluxWeight[i] = weight[i];
    }
}
...
double BANFFEventBase::GetFluxWeight(){

  return fluxWeight[sample->sampleID];
...
void BANFFEventBase::SaveTotalWeightFast(FitParameters* fitParameters, double Q2, int NuFlavor){
    ...
    if(!(fitParameters->continuousParamsComputed)){
        ...
        xsecWeightNoDiscreteParams = 1.0;
        //The fluxWeight and the POT weight are always included.
        totalWeightNoDiscreteParams = fluxWeight[sample->sampleID] * POTWeight;
...
void BANFFEventBase::CopySummaryTreeInfo(){
    ...
    for(int i = 0; i < nSamples; i++){
        ...
        fluxWeight[i] = fluxWeight[sourceIndex];
