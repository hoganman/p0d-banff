#define THROWPARAMS_CXX
#include "ThrowParams.hxx"
ClassImp(ThrowParams)
#include "TDatime.h"
#include "TMath.h"
#include<iostream>
...
ThrowParams::ThrowParams(TVectorT<double> &params, TMatrixTSym<double> &covm)
{
...
ThrowParams::~ThrowParams()
{
    ...
    if(chel_dec) chel_dec->Delete();
}

void ThrowParams::SetSeed(int seed)
{
    if(seed == 0)
    {
        TDatime now;
...
void ThrowParams::ThrowSet(std::vector<double> &params)
{
   if(!params.empty())
       params.clear();
   params.resize(npars);
   ...
   for(int i=0;i<npars;i++)
       params[i] = prod(i) + (*pvals)(i);
}

void ThrowParams::StdNormRand(double *z)
{
...
void ThrowParams::CheloskyDecomp(TMatrixT<double> &chel_mat)
{
    for(int i=0; i<npars; i++)
