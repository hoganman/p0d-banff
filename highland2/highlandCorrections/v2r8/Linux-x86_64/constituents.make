...
ifdef READONLY
cmt_local_tagfile = /tmp/CMT_$(highlandCorrections_tag).make$(cmt_lock_pid)
else
#cmt_local_tagfile = $(highlandCorrections_tag).make
cmt_local_tagfile = $(bin)$(highlandCorrections_tag).make
endif
...
ifdef READONLY
cmt_local_setup = /tmp/CMT_highlandCorrectionssetup$(cmt_lock_pid).make
cmt_final_setup = /tmp/CMT_highlandCorrectionssetup.make
else
#cmt_local_setup = $(bin)highlandCorrectionssetup$(cmt_lock_pid).make
...
ifdef READONLY
cmt_local_tagfile_doxygen = /tmp/CMT_$(highlandCorrections_tag)_doxygen.make$(cmt_lock_pid)
cmt_final_setup_doxygen = /tmp/CMT_highlandCorrections_doxygensetup.make
cmt_local_doxygen_makefile = /tmp/CMT_doxygen$(cmt_lock_pid).make
else
#cmt_local_tagfile_doxygen = $(highlandCorrections_tag)_doxygen.make
cmt_local_tagfile_doxygen = $(bin)$(highlandCorrections_tag)_doxygen.make
cmt_final_setup_doxygen = $(bin)highlandCorrections_doxygensetup.make
...
ifdef READONLY
cmt_local_tagfile_doxygen = /tmp/CMT_$(highlandCorrections_tag).make$(cmt_lock_pid)
cmt_final_setup_doxygen = /tmp/CMT_highlandCorrectionssetup.make
cmt_local_doxygen_makefile = /tmp/CMT_doxygen$(cmt_lock_pid).make
else
#cmt_local_tagfile_doxygen = $(highlandCorrections_tag).make
cmt_local_tagfile_doxygen = $(bin)$(highlandCorrections_tag).make
cmt_final_setup_doxygen = $(bin)highlandCorrectionssetup.make
cmt_local_doxygen_makefile = $(bin)doxygen.make
endif
...
highlandCorrections_extratags = -tag_add=target_highlandCorrections

#$(cmt_local_tagfile_highlandCorrections) : $(cmt_lock_setup)
ifndef QUICK
$(cmt_local_tagfile_highlandCorrections) ::
else
$(cmt_local_tagfile_highlandCorrections) :
endif
	$(echo) "(constituents.make) Rebuilding setup.make $(cmt_local_tagfile_highlandCorrections)"
	@if test -f $(cmt_local_tagfile_highlandCorrections); then /bin/rm -f $(cmt_local_tagfile_highlandCorrections); fi ; \
	  $(cmtexe) -tag=$(tags) $(highlandCorrections_extratags) build tag_makefile >>$(cmt_local_tagfile_highlandCorrections); \
	  if test -f $(cmt_final_setup_highlandCorrections); then /bin/rm -f $(cmt_final_setup_highlandCorrections); fi; \
	  $(cmtexe) -tag=$(tags) $(highlandCorrections_extratags) show setup >>$(cmt_final_setup_highlandCorrections)
	$(echo) setup.make ok
...
ifndef QUICK
$(cmt_local_highlandCorrections_makefile) :: $(highlandCorrections_dependencies) $(cmt_local_tagfile_highlandCorrections) build_library_links dirs
else
$(cmt_local_highlandCorrections_makefile) :: $(cmt_local_tagfile_highlandCorrections)
endif
	$(echo) "(constituents.make) Building highlandCorrections.make"; \
	  $(cmtexe) -tag=$(tags) $(highlandCorrections_extratags) build constituent_makefile -out=$(cmt_local_highlandCorrections_makefile) highlandCorrections

highlandCorrections :: $(highlandCorrections_dependencies) $(cmt_local_highlandCorrections_makefile)
	$(echo) "(constituents.make) Starting highlandCorrections"
	@$(MAKE) -f $(cmt_local_highlandCorrections_makefile) cmt_lock_pid=$${cmt_lock_pid} highlandCorrections
	$(echo) "(constituents.make) highlandCorrections done"

clean :: highlandCorrectionsclean

highlandCorrectionsclean :: $(highlandCorrectionsclean_dependencies) ##$(cmt_local_highlandCorrections_makefile)
	$(echo) "(constituents.make) Starting highlandCorrectionsclean"
	@-if test -f $(cmt_local_highlandCorrections_makefile); then \
	  $(MAKE) -f $(cmt_local_highlandCorrections_makefile) cmt_lock_pid=$${cmt_lock_pid} highlandCorrectionsclean; \
	fi

##	  /bin/rm -f $(cmt_local_highlandCorrections_makefile) $(bin)highlandCorrections_dependencies.make

install :: highlandCorrectionsinstall

highlandCorrectionsinstall :: $(highlandCorrections_dependencies) $(cmt_local_highlandCorrections_makefile)
	$(echo) "(constituents.make) Starting install highlandCorrections"
	@-$(MAKE) -f $(cmt_local_highlandCorrections_makefile) cmt_lock_pid=$${cmt_lock_pid} install
	$(echo) "(constituents.make) install highlandCorrections done"

uninstall :: highlandCorrectionsuninstall

highlandCorrectionsuninstall :: $(cmt_local_highlandCorrections_makefile)
	$(echo) "(constituents.make) Starting uninstall highlandCorrections"
	@-$(MAKE) -f $(cmt_local_highlandCorrections_makefile) cmt_lock_pid=$${cmt_lock_pid} uninstall
	$(echo) "(constituents.make) uninstall highlandCorrections done"
...
ifdef READONLY
cmt_local_tagfile_make = /tmp/CMT_$(highlandCorrections_tag)_make.make$(cmt_lock_pid)
cmt_final_setup_make = /tmp/CMT_highlandCorrections_makesetup.make
cmt_local_make_makefile = /tmp/CMT_make$(cmt_lock_pid).make
else
#cmt_local_tagfile_make = $(highlandCorrections_tag)_make.make
cmt_local_tagfile_make = $(bin)$(highlandCorrections_tag)_make.make
cmt_final_setup_make = $(bin)highlandCorrections_makesetup.make
...
ifdef READONLY
cmt_local_tagfile_make = /tmp/CMT_$(highlandCorrections_tag).make$(cmt_lock_pid)
cmt_final_setup_make = /tmp/CMT_highlandCorrectionssetup.make
cmt_local_make_makefile = /tmp/CMT_make$(cmt_lock_pid).make
else
#cmt_local_tagfile_make = $(highlandCorrections_tag).make
cmt_local_tagfile_make = $(bin)$(highlandCorrections_tag).make
cmt_final_setup_make = $(bin)highlandCorrectionssetup.make
cmt_local_make_makefile = $(bin)make.make
endif
