void CreateEffPur(const char* defaultTreeFilename)
{
    ...
    draw.SetTitleY("#nu_{#mu} CCInc Eff/Purity");
    draw.DrawEff(truth, "truelepton_mom*1e-3", 50, 0, 5, "accum_level[]>4", "reactionCC==1", "", "", "#nu_{#mu}CC Efficiency");
    draw.DrawEff(default, "truelepton_mom*1e-3", 50, 0, 5, "reactionCC==1", "accum_level[]>4", "SAME", "", "#nu_{#mu}CC Purity");

    canvas->SaveAs("numuWaterOutFHCCC_recoP_effpur_reactionCC.root");
    canvas->SaveAs("numuWaterOutFHCCC_recoP_effpur_reactionCC.png");

    draw.SetTitleX("True muon (1-cos#theta)");
    draw.SetTitleY("#nu_{#mu} CCInc Eff/Purity");
    draw.DrawEff(truth, "1-truelepton_costheta", 50, 0, 1, "accum_level[]>4", "reactionCC==1", "", "", "#nu_{#mu}CCQE Efficiency");
    draw.DrawEff(default, "1-truelepton_costheta", 50, 0, 1, "reactionCC==1", "accum_level[]>4", "SAME", "", "#nu_{#mu}CCQE Purity");

    canvas->SaveAs("numuWaterOutFHCCC_recocostheta_effpur_reactionCC.root");
    canvas->SaveAs("numuWaterOutFHCCC_recocostheta_effpur_reactionCC.png");
