...
#  Document dictionary
#
#   Generated Mon Oct  9 06:38:57 2017  by mhogan
#
#====================================
...
dictionary :: $(filter %_LinkDef.rootcint,$(dictionary_output)DataClasses_LinkDef.rootcint $(dictionary_output)P0DDataClasses_LinkDef.rootcint)
	@echo "------> dictionary ok"

DataClasses_LinkDef_h_dependencies = ../src/DataClasses_LinkDef.h
P0DDataClasses_LinkDef_h_dependencies = ../src/P0DDataClasses_LinkDef.h
# -*- makefile -*-
# 
# A makefile fragment used by CMT to build the ROOT dictionary files.
#

$(dict)DataClasses_LinkDef.rootcint : $(dict)$(patsubst %_LinkDef,%_Dict,DataClasses_LinkDef).cxx
	@touch $@

$(dict)$(patsubst %_LinkDef,%_Dict,DataClasses_LinkDef).cxx : \
    $(patsubst %_LinkDef.h,%.h*,$(src)DataClasses_LinkDef.h) \
    $(src)DataClasses_LinkDef.h  $(ROOT_stamps)
...
$(dict)P0DDataClasses_LinkDef.rootcint : $(dict)$(patsubst %_LinkDef,%_Dict,P0DDataClasses_LinkDef).cxx
	@touch $@

$(dict)$(patsubst %_LinkDef,%_Dict,P0DDataClasses_LinkDef).cxx : \
    $(patsubst %_LinkDef.h,%.h*,$(src)P0DDataClasses_LinkDef.h) \
    $(src)P0DDataClasses_LinkDef.h  $(ROOT_stamps)
	@echo "Generating ROOT Dictionary from " $(filter-out %~,$^)
	@if [ ! -d $(dict) ]; then mkdir $(dict); fi ;
	$(rootcint) -f $@ -c -p $(includes) $(cppflags) $(word 1, $(filter-out %~,$^)) $(word 2, $(filter-out %~,$^))
