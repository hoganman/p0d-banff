void numuMultiPiDrawRelativeError(){
  ...
  DrawingTools* draw = new DrawingTools("test.root",4);
  ...
  //=== Magnet MC samples
  DataSample* magnet_mc_run2w = new DataSample("test.root");
 // DataSample* magnet_mc_run2a  = new DataSample("/lustrehome/magaletti/results/MC/Dev_Run2a_6B_highlad2_FlatTree/Run2a_fgd1_single_syst_200thr.root");
...
//  run2a.AddMCSample("magnet",magnet_mc_run2a);
  ...
  exp_fgd1.AddSampleGroup( "run2w" , run2w );
  //exp_fgd1.AddSampleGroup( "run2a" , run2a );

  //=== Initialize Experiment class 
  Experiment exp_fgd2("nd280");

  //=== Magnet MC samples
  DataSample* magnet_mc_run2w_fgd2 = new DataSample("test.root");
  //DataSample* magnet_mc_run2a_fgd2  = new DataSample("/lustrehome/magaletti/results/MC/Dev_Run2a_6B_highlad2_FlatTree/Run2a_fgd2_single_syst_200thr.root");

  //=== Run2w sample
  SampleGroup run2w_fgd2("run2w");
  run2w_fgd2.AddMCSample("magnet",magnet_mc_run2w_fgd2);

  //=== Run2a sample
//  SampleGroup run2a_fgd2("runa2");
 // run2a_fgd2.AddMCSample("magnet",magnet_mc_run2a_fgd2);
  ...
  //run2w_fgd2.AddMCSample("sand",sand_mc_run5_fgd2);
  //run2a_fgd2.AddMCSample("sand",sand_mc_run6_fgd2);  

  exp_fgd2.AddSampleGroup( "run2w" , run2w_fgd2 );
  //exp_fgd2.AddSampleGroup( "run2a"  , run2a_fgd2  );

  const int NSamples = 3;
  string cut_sample[NSamples]  = { "accum_level[][0]>6" , "accum_level[][1]>6" , "accum_level[][2]>6" };
  ...
  for(int isy=0; isy<NSyst; isy++){
    ...
    //if(_syst_name!="sandmu_syst" && _syst_name!="all_syst"){
    if(_syst_name=="sipion_syst"){
      exp_fgd1.GetMCSample("run2w" , "magnet")->SetCurrentTree(syst_name[isy]);
     // exp_fgd1.GetMCSample("run2a"  , "magnet")->SetCurrentTree(syst_name[isy]);
      exp_fgd2.GetMCSample("run2w" , "magnet")->SetCurrentTree(syst_name[isy]);
    //  exp_fgd2.GetMCSample("run2a"  , "magnet")->SetCurrentTree(syst_name[isy]);
    }
    else{
      continue;
    ...
    for (int is=0; is<NSamples; is++){
      ...
      _cut=cut_sample[is];
      if(isy==NSyst-1)
        draw->DrawRelativeErrors(exp_fgd1,"selmu_mom/1000",20,0,5,(_cut+" && weight_syst_total<10 && weight_syst_total>=0").c_str(),"",(_opt+" NODRAW").c_str());
      else
        draw->DrawRelativeErrors(exp_fgd1,"selmu_mom/1000",20,0,5,(_cut+" && weight_syst_total<10 && weight_syst_total>=0").c_str(),"",(_opt+" NODRAW").c_str());
      ...
      if(isy==NSyst-1)
        draw->DrawRelativeErrors(exp_fgd2,"selmu_mom/1000",20,0,5,(_cut+" && weight_syst_total<10 && weight_syst_total>=0").c_str(),"",(_opt+" NODRAW").c_str()); 
      else
        draw->DrawRelativeErrors(exp_fgd2,"selmu_mom/1000",20,0,5,(_cut+" && weight_syst_total<10 && weight_syst_total>=0").c_str(),"",(_opt+" NODRAW").c_str());
